...
cell AMX_NATIVE_CALL n_TrackVehicle(AMX* amx, cell* params)
{
...
cell AMX_NATIVE_CALL n_UntrackVehicle(AMX* amx, cell* params)
{
    auto* v = VehicleSystem::getInstance().get(params[1]);
    if (!v) return 0;
    v->flags &= ~VEHICLE_FLAG_TRACKED;
...
cell AMX_NATIVE_CALL n_GetVehicleLastPosition(AMX* amx, cell* params)
{
    auto* v = VehicleSystem::getInstance().get(params[1]);
    if (!v) return 0;
...
cell AMX_NATIVE_CALL n_GetVehicleTravelDistance(AMX* amx, cell* params)
{
    auto* v = VehicleSystem::getInstance().get(params[1]);
    if (!v) return 0;
...
cell AMX_NATIVE_CALL n_GetVehicleDistanceFromPoint(AMX* amx, cell* params)
{
    auto* v = VehicleSystem::getInstance().get(params[1]);
    if (!v) return 0;
