...
cell AMX_NATIVE_CALL n_CreateFleet(AMX* amx, const cell* params)
{
    char name[128];
...
cell AMX_NATIVE_CALL n_DestroyFleet(AMX* amx, const cell* params)
{
    return VehicleSystem::getInstance().destroyFleet((int)params[1]) ? 1 : 0;
}

/* native AssignVehicleToFleet(vehicleid, fleetid); */
cell AMX_NATIVE_CALL n_AssignVehicleToFleet(AMX* amx, const cell* params)
{
...
cell AMX_NATIVE_CALL n_RemoveVehicleFromFleet(AMX* amx, const cell* params)
{
    auto* v = VehicleSystem::getInstance().get(params[1]);
    if (!v) return 0;
    int old = v->fleetid;
...
cell AMX_NATIVE_CALL n_GetFleetVehicleCount(AMX* amx, const cell* params)
{
...
cell AMX_NATIVE_CALL n_GetVehicleFleet(AMX* amx, const cell* params)
{
    auto* v = VehicleSystem::getInstance().get(params[1]);
...
cell AMX_NATIVE_CALL n_GetFleetOwner(AMX* amx, const cell* params)
{
...
cell AMX_NATIVE_CALL n_IsVehicleInFleet(AMX* amx, const cell* params)
{
    auto* v = VehicleSystem::getInstance().get(params[1]);
