...
cell AMX_NATIVE_CALL n_GiveVehicleKey(AMX* amx, cell* params)
{
...
cell AMX_NATIVE_CALL n_RemoveVehicleKey(AMX* amx, cell* params)
{
    if (!VehicleSystem::getInstance().isManaged(params[1])) return 0;
...
cell AMX_NATIVE_CALL n_HasVehicleKey(AMX* amx, cell* params)
{
...
cell AMX_NATIVE_CALL n_LockVehicle(AMX* amx, cell* params)
{
    auto* v = VehicleSystem::getInstance().get(params[1]);
    if (!v) return 0;
...
cell AMX_NATIVE_CALL n_UnlockVehicle(AMX* amx, cell* params)
{
    auto* v = VehicleSystem::getInstance().get(params[1]);
    if (!v) return 0;
    v->flags &= ~VEHICLE_FLAG_LOCKED;
...
cell AMX_NATIVE_CALL n_GetVehicleKeyCount(AMX* amx, cell* params)
{
    auto it = g_vehicleKeys.find(params[1]);
    if (it == g_vehicleKeys.end()) return 0;
...
cell AMX_NATIVE_CALL n_ClearVehicleKeys(AMX* amx, cell* params)
{
