...
cell AMX_NATIVE_CALL n_InsureVehicle(AMX* amx, cell* params)
{
...
cell AMX_NATIVE_CALL n_CancelInsurance(AMX* amx, cell* params)
{
    auto* v = VehicleSystem::getInstance().get(params[1]);
    if (!v) return 0;
    v->insuranceExpiry = 0;
...
cell AMX_NATIVE_CALL n_IsVehicleInsured(AMX* amx, cell* params)
{
    auto* v = VehicleSystem::getInstance().get(params[1]);
    if (!v) return 0;
...
cell AMX_NATIVE_CALL n_GetInsuranceExpiration(AMX* amx, cell* params)
{
    auto* v = VehicleSystem::getInstance().get(params[1]);
...
cell AMX_NATIVE_CALL n_RenewInsurance(AMX* amx, cell* params)
{
    auto* v = VehicleSystem::getInstance().get(params[1]);
    if (!v) return 0;
