...
cell AMX_NATIVE_CALL n_EVF9_SetVehicleFlag(AMX* amx, const cell* params)
{
...
cell AMX_NATIVE_CALL n_EVF9_ClearVehicleFlag(AMX* amx, const cell* params)
{
    auto* v = VehicleSystem::getInstance().get(params[1]);
    if (!v) return 0;
    v->flags &= ~(uint32_t)params[2];
...
cell AMX_NATIVE_CALL n_EVF9_HasVehicleFlag(AMX* amx, const cell* params)
{
    auto* v = VehicleSystem::getInstance().get(params[1]);
    if (!v) return 0;
